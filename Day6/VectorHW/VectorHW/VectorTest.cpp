...
int main()
{
  ...
  */
  ...
  // TODO: Add your own tests that thoroughly exercise your VectorUtil library.
    if( FindMin(v1) != -1) {
      ErrorExit("FindMin() - v1");
    }
    if( FindMin(v2) != 1) {
      ErrorExit("FindMin() - v2");
    }
    if( FindMax(v1) != 5) {
      ErrorExit("FindMax() - v1");
    }
    if( FindMax(v2) != 3) {
      ErrorExit("FindMax() - v2");
    }
    if( Average(v1) != 1) {
      ErrorExit("Average() - v1");
    }
    if( Average(v2) != 2) {
        ErrorExit("Average() - v2");
      }
    if( IsSorted(v1) == true) {
        ErrorExit("IsSorted() - v1");
      }
    if( IsSorted(v2) == false) {
        ErrorExit("IsSorted() - v2");
      }
